    ...
    public class Charge : My
    {
        ...
        protected override List<AttackObject> ConventionalAttack(byte[] key, bool start)
        {
            ...
            if (start)
            {
                ...
                attack.Add(NewBullet(this.BulletDotC, 16, new Vec(0, -10), DXImage.Instance.bulletMedium[0], this.AttackObjectMeta));
            }
            else
            {
        ...
        protected override List<AttackObject> SpecialAttack(byte[] key, bool start)
        {
            ...
            else
            {
                if (key[Config.Instance.key[KeyComfig.GAME_SPECIAL]] == DX.FALSE && this.specialAttack >= (this.Strengthen == 0 ? 30 : 15))
                {
                    ...
                    {
                        attack.Add(NewBullet(this.BulletDotC, 66, new Vec(0, -10), DXImage.Instance.bulletSmall[0], this.AttackObjectMeta));
                    }
                    else if (this.specialAttack < (this.Strengthen == 0 ? 180 : 89))//120～179|60～90
                    {
                        attack.Add(NewBullet(this.BulletDotC, 144, new Vec(0, -10), DXImage.Instance.bulletMedium[0], this.AttackObjectMeta));
                    }
                    else//180～|90～
                    {
                        attack.Add(NewBullet(this.BulletDotC, 234, new Vec(0, -10), DXImage.Instance.bulletBig[0], this.AttackObjectMeta));
                    }
                    this.Action = MyAction.NONE;
                }
                this.specialAttack++;
        ...
        protected override List<AttackObject> Deathblow(byte[] key, bool start)
        {
            ...
            else
            {
                ...
                {
                    attack = new List<AttackObject>();
                    attack.Add(NewBullet(this.BulletDotC, 468, new Vec(0, -10), DXImage.Instance.bomb, this.AttackObjectMeta));
                    this.Action = MyAction.NONE;
                }
            }
            this.deathblow++;
